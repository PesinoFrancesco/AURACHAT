...
def log_to_xml(log_filename, level, log_type, message, sender=None, receiver=None):
    ...
    with xml_lock:
        try:
            ...
            # NUOVO: Aggiungi attributi sender e receiver
            if sender:
            ...
            # NUOVO: Mostra mittente e destinatario nel log console
            direzione = ""
            if sender and receiver:
                direzione = f" [{sender} → {receiver}]"
            elif sender:
                direzione = f" [da: {sender}]"
            elif receiver:
                direzione = f" [a: {receiver}]"
                
            if log_type in ["CONNECTION", "ERROR", "DISCONNECTION", "AUTH", "REGISTRATION", "MESSAGE_RECEIVED", "MESSAGE_SENT"]:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] [{level_colored}]{direzione} {message}")
...
def gestisci_comando_info(comando_completo, client_address, username):
    ...
    if info_type == 1:
        num_client = len(utenti_connessi)
        risposta = (
...
def autenticazione(mio_socket, client_address, log_filename):
    ...
    try:
        ...
        if risposta == "SI":
                ...
                if verifica_credenziali(username, password):
                    ...
                    with utenti_lock:
                        if username in utenti_connessi:
                            ...
                            log_to_xml(log_filename, "WARNING", "AUTH", f"Tentativo login con utente già connesso: {username}",
                                      sender=username, receiver="SERVER")
                            return False, None
                    
                    aggiorna_ultimo_accesso(username)
                    mio_socket.send(f"AUTH_SUCCESS|Benvenuto {username}!".encode('utf-8'))
                    
                    log_to_xml(log_filename, "INFO", "AUTH", f"Login effettuato: {username}",
                              sender=username, receiver="SERVER")
                    ...
                    log_to_xml(log_filename, "WARNING", "AUTH", f"Tentativo login fallito per username: {username}",
                              sender=username, receiver="SERVER")
            ...
            log_to_xml(log_filename, "INFO", "REGISTRATION", f"Nuovo utente registrato: {username}",
                      sender=username, receiver="SERVER")
        ...
        log_to_xml(log_filename, "ERROR", "AUTH_ERROR", f"Errore durante autenticazione: {e}",
                  sender=client_address[0], receiver="SERVER")
        return False, None
...
def gestisci_client(mio_socket, client_address, log_filename):
    ...
    if not auth_ok:
        log_to_xml(log_filename, "WARNING", "AUTH_FAILED", f"Autenticazione fallita per {client_id}",
                  sender=client_id, receiver="SERVER")
        mio_socket.close()
        return
    ...
    log_to_xml(log_filename, "INFO", "CONNECTION", f"Utente {username} connesso da {client_id}",
              sender=username, receiver="SERVER")
    ...
    try:
        while server_running:
            ...
            # NUOVO: Log messaggio ricevuto dal client
            log_to_xml(log_filename, "INFO", "MESSAGE_RECEIVED", 
                      f"Comando: {data}",
                      sender=username, receiver="SERVER")
            ...
            if comando == "TIME":
                ...
                # NUOVO: Log risposta inviata al client
                log_to_xml(log_filename, "INFO", "MESSAGE_SENT", 
                          f"Risposta TIME: {risposta}",
                          sender="SERVER", receiver=username)
                ...
                # NUOVO: Log risposta inviata
                log_to_xml(log_filename, "INFO", "MESSAGE_SENT", 
                          f"Risposta NAME: {risposta}",
                          sender="SERVER", receiver=username)
                ...
                # NUOVO: Log risposta EXIT
                log_to_xml(log_filename, "INFO", "MESSAGE_SENT", 
                          f"Risposta EXIT: {risposta}",
                          sender="SERVER", receiver=username)
                break
                ...
                # NUOVO: Log risposta INFO
                log_to_xml(log_filename, "INFO", "MESSAGE_SENT", 
                          f"Risposta INFO: {risposta[:100]}...",  # Tronca se troppo lungo
                          sender="SERVER", receiver=username)
                ...
                try:
                    ...
                    # NUOVO: Log invio file log
                    log_to_xml(log_filename, "INFO", "MESSAGE_SENT",
                              f"File log inviato ({len(contenuto_log)} bytes)",
                              sender="SERVER", receiver=username)
                    
                except Exception as e:
                    errore = f"LOG_ERROR|Errore lettura log: {e}"
                    mio_socket.send(errore.encode('utf-8'))
                    log_to_xml(log_filename, "ERROR", "MESSAGE_SENT",
                              f"Errore invio log: {e}",
                              sender="SERVER", receiver=username)
                ...
                # NUOVO: Log comando non riconosciuto
                log_to_xml(log_filename, "INFO", "MESSAGE_SENT", 
                          f"Comando non riconosciuto: {risposta}",
                          sender="SERVER", receiver=username)

                
                
    except ConnectionResetError:
        log_to_xml(log_filename, "WARNING", "CONNECTION_ERROR", f"{Colori.ROSSO}Connessione interrotta da {username}",
                  sender=username, receiver="SERVER")
    except Exception as e:
        log_to_xml(log_filename, "ERROR", "EXCEPTION", f"{Colori.ROSSO}Errore con utente {username}: {e}",
                  sender=username, receiver="SERVER")
    finally:
        ...
        log_to_xml(log_filename, "INFO", "THREAD_CLOSE", f"Chiusura thread per {username}",
                  sender=username, receiver="SERVER")
...
if __name__ == "__main__":
    ...
    log_to_xml(log_filename, "INFO", "SERVER_START", "Server TCP avviato con sistema autenticazione")
    ...
    try:
        ...
        log_to_xml(log_filename, "ERROR", "FATAL_ERROR", f"Errore fatale server: {e}")
    finally:
        mio_server.close()
        log_to_xml(log_filename, "INFO", "SERVER_SHUTDOWN", "Server chiuso correttamente")
