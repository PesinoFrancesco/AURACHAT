...
def gestisci_client(mio_socket, client_address, log_filename):
    ...
    try:
        while server_running:
            ...
            if not data:
                log_to_xml(log_filename, "INFO", "DISCONNECTION", f"Utente {username} ha chiuso la connessione")
                break
                
            log_to_xml(log_filename, "INFO", "REQUEST", f"Richiesta '{data}' da {username}")
            ...
            if comando == "TIME":
                ...
                mio_socket.send(risposta.encode('utf-8'))
                log_to_xml(log_filename, "INFO", "EXIT_REQUEST", f"EXIT richiesto da {username}")
                break
                ...
                try:
                    # Leggi il file XML di log del server (con lock per evitare conflitti)
                    with xml_lock:
                        with open(log_filename, 'r', encoding='utf-8') as f:
                            contenuto_log = f.read()
                    
                    # Aggiungi marcatore di fine
                    contenuto_completo = contenuto_log + "\n<<<END_OF_LOG>>>"
                    
                    # Invia direttamente il contenuto
                    mio_socket.send(contenuto_completo.encode('utf-8'))
                    
                    log_to_xml(log_filename, "INFO", "LOG_SENT",
                            f"Log inviati a {username} ({len(contenuto_log)} bytes)")
                    
                    print(f"{Colori.VERDE}[LOG] File inviato a {username} ({len(contenuto_log)} bytes){Colori.RESET}")
                    
                except FileNotFoundError:
                    errore = "LOG_ERROR|File log non trovato"
