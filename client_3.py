...
def get_info_client(mio_socket):
    """Restituisce informazioni di rete del client"""
    hostname = socket.gethostname()
    try:
        # IP locale del client
        ip_address = socket.gethostbyname(hostname)
    except:
        ip_address = "N/A"
    
    # Ottieni info sulla connessione al server
    try:
...
║  IP Locale:       {ip_address:<40}║
║  Porta Locale:    {local_port!s:<40}║
║  Server IP:       {server_ip:<40}║
║  Server Porta:    {server_port!s:<40}║
║  Sistema:         {sistema:<40}║
...
def main():
    ...
    try:
        while True:
            ...
            # Ricezione risposta
            try:
                data = mio_socket.recv(4096).decode('utf-8', errors='ignore')
                ...
                if data == "CLIENT_HANDLE_INFO_4":
                    # Il client elabora le proprie informazioni di rete
                    ...
                    # Risposta normale dal server
                    log_to_xml(log_filename, "INFO", "RECEIVE",
                              f"Ricevuta risposta dal server",
                              response=data[:100],  # Solo primi 100 caratteri nel log
                              client_name=client_name)
