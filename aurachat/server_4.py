...
def gestisci_client(mio_socket, client_address, log_filename):
    ...
    if not auth_ok:
        logger.log_to_xml(log_filename, "WARNING", "AUTH_FAILED", f"Autenticazione fallita per {client_id}")
        mio_socket.close()
        return
    ...
    logger.log_to_xml(log_filename, "INFO", "CONNECTION", f"Utente {username} connesso da {client_id}")
    ...
    try:
        while server_running:
            ...
            if not data:
                logger.log_to_xml(log_filename, "INFO", "DISCONNECTION", f"Utente {username} ha chiuso la connessione")
                break
                
            logger.log_to_xml(log_filename, "INFO", "REQUEST", f"Richiesta '{data}' da {username}")
            ...
            if comando == "TIME":
                ...
                mio_socket.send(risposta.encode('utf-8'))
                logger.log_to_xml(log_filename, "INFO", "EXIT_REQUEST", f"EXIT richiesto da {username}")
                break
        ...
        logger.log_to_xml(log_filename, "WARNING", "CONNECTION_ERROR", f"{Colori.ROSSO}Connessione interrotta da {username}")
    except Exception as e:
        logger.log_to_xml(log_filename, "ERROR", "EXCEPTION", f"{Colori.ROSSO}Errore con utente {username}: {e}")
    finally:
        ...
        logger.log_to_xml(log_filename, "INFO", "THREAD_CLOSE", f"Chiusura thread per {username}")
...
if __name__ == "__main__":
    ...
    log_filename = logger.setup_xml_log()
    print(f"Log XML: {log_filename}")
    print(f"Config utenti: {authenticator.CONFIG_FILE}")
    print(f"Token Discovery: {SECRET_TOKEN}")
    
    logger.log_to_xml(log_filename, "INFO", "SERVER_START", "Server TCP avviato con sistema autenticazione")
    ...
    try:
        ...
        logger.log_to_xml(log_filename, "ERROR", "FATAL_ERROR", f"Errore fatale server: {e}")
    finally:
        mio_server.close()
        logger.log_to_xml(log_filename, "INFO", "SERVER_SHUTDOWN", "Server chiuso correttamente")
