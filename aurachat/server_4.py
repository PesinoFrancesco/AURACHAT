    ...
    try:
        while server_running:
            ...
            if comando == "TIME":
                ...
                data_log = "OGGI" # Per il messaggio di log finale

                # 1. IDENTIFICAZIONE DEL FILE DA INVIARE
                if len(parti) == 1:
                    # Caso: LOG (senza data) -> File attuale
                    target_filename = client_log_filename
                
                elif len(parti) == 2:
                    # Caso: LOG 2026-02-05 -> File storico
                    data = parti[1]
                    data_log = data
                    cartella_logs = os.path.dirname(client_log_filename)
                    nome_file_semplice = f"client_{username}_{data}.xml"
                    target_filename = os.path.join(cartella_logs, nome_file_semplice)
                
                else:
                    # Caso: Comando sbagliato
                    mio_socket.send("Comando LOG non valido (Usa: LOG oppure LOG AAAA-MM-GG)".encode('utf-8'))
                    logger.log_to_xml(client_log_filename, "WARNING", "INVALID_COMMAND", f"Comando LOG errato: {comando}")
                    # Usiamo 'continue' o saltiamo il resto del codice per questo giro
                    target_filename = None 

                # 2. PROCEDURA DI INVIO FILE (Se abbiamo un file valido)
                if target_filename:
                    if os.path.exists(target_filename):
                        try:
                            # A. Leggiamo il file in modalità BINARIA (rb)
                            with open(target_filename, "rb") as f:
                                file_content = f.read()
                            
                            dimensione = len(file_content)

                            # B. Inviamo l'intestazione: "Sto per mandare un file grande X"
                            header = f"FILE_START:{dimensione}"
                            mio_socket.send(header.encode('utf-8'))

                            # C. ASPETTIAMO che il client dica "READY" (Sincronizzazione)
                            # Questo evita che il client riceva i dati mentre ancora processa l'header
                            conferma = mio_socket.recv(1024).decode('utf-8')

                            if conferma.strip() == "READY":
                                # D. Inviamo tutto il file
                                mio_socket.sendall(file_content)
                                
                                # Log successo
                                logger.log_to_xml(client_log_filename, "INFO", "RESPONSE", f"File log inviato ({data_log}) - {dimensione} bytes")
                            else:
                                # Il client non ha risposto READY
                                print(f"Errore: Client non pronto. Ha risposto: {conferma}")

                        except Exception as e:
                            msg_errore = f"Errore server durante l'invio: {str(e)}"
                            mio_socket.send(msg_errore.encode('utf-8'))
                            logger.log_to_xml(client_log_filename, "ERROR", "SEND_ERROR", msg_errore)
                    else:
                        # File non trovato
                        msg = f"Nessun log trovato per: {data_log}"
                        mio_socket.send(msg.encode('utf-8'))
                        logger.log_to_xml(client_log_filename, "WARNING", "NOT_FOUND", msg)

            else:
                risposta = f"Comando '{data}' non riconosciuto. Comandi: TIME, NAME, INFO, LOG, EXIT"
                mio_socket.send(risposta.encode('utf-8'))
                logger.log_to_xml(client_log_filename, "WARNING", "UNKNOWN_COMMAND", f"Comando non riconosciuto: {data}")
